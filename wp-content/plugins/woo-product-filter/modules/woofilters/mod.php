	...
	public function forceProductFilter( $query ) {
		...
		if ( isset( $query->query_vars['post_type'] ) && 'product' === $query->query_vars['post_type'] && function_exists( 'debug_backtrace' ) ) {
			...
			if ( is_array( $backtrace ) ) {
				...
				$isDynamicContentForElementor = ( isset( $backtrace[7]['class'] ) && 'DynamicContentForElementor\Widgets\DCE_Widget_DynamicPosts_Base' === $backtrace[7]['class'] );

				$found = $isBlocks || $isAddons || $isElementorPostQuery || $isDynamicContentForElementor;
				if (!$found) {
